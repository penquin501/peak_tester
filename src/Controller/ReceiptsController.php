...
//        $entityManager = $this->getDoctrine()->getManager('default');
    ...
    /**
     * @Route("/save/ecom/receipts/info",name="save_ecom_receipts")
     */
    public function saveEcomReceiptsInfo(Request $request)
    {
        $datePrepare = $request->query->get('datePrepare');
        $testDate = $this->convertStrToDate($datePrepare);
        $nextDate = date('Y-m-d', strtotime("+1 day", strtotime($testDate)));
        ...
        $sqlParcelTest = "SELECT bill_no FROM ecom_receipts";
        $billNoExisted = $customEntityManager->getConnection()->query($sqlParcelTest);
        $dataBillNoExisted = json_decode($this->json($billNoExisted)->getContent(), true);
            ...
            foreach ($dataPeak as $item) {
                if ($item['peak_status'] != 'recorded') {
                    ...
                    foreach ($dataJsonSend['PeakReceipts']['receipts'] as $itemReceipts) {
                        ...
                        $strBillNo = $itemReceipts['tags'][2];
                        $exBillNo = explode("|", $strBillNo);//$exBillNo[1]

                        $strShopName = $itemReceipts['tags'][4];
                        $exShopName = explode("|", $strShopName);//$exShopName[1]
...
//                        dd($merId[0]);
                        ...
                        $amount = $itemReceipts['paidPayments']['payments'][0]['amount'];
                        foreach ($itemReceipts['products'] as $product) {
                            $insertProductItem = "INSERT INTO ecom_receipts_send_item(bill_no,item_date, product_id, quantity, price) VALUES " .
                                "('" . $exBillNo[1] . "','" . $issuedDate . "','" . $product['productId'] . "'," . $product['quantity'] . "," . $product['price'] . ")";
                            $customEntityManager->getConnection()->query($insertProductItem);
                        }
                    }

                    if ($dataJsonPeak == '' || !is_array($dataJsonPeak)) {
                        $insertQuery = "INSERT INTO ecom_receipts(code, issued_date, bill_no, mer_id, shop_name, amount_send,peak_due_date,record_date) " .
                            "VALUES ('" . $code . "','" . $issueDateToDate . "','" . $exBillNo[1] . "'," . $merId[0] . ",'" . $exShopName[1] . "'," . $amount . ",'" . $dueDateToDate . "',CURRENT_TIMESTAMP())";
                    } elseif ($dataJsonPeak['PeakReceipts']['resCode'] != 200) {
                        $insertQuery = "INSERT INTO ecom_receipts(code, issued_date, bill_no,mer_id, shop_name, amount_send,peak_due_date,peak_res_code,peak_res_desc,record_date) " .
                            "VALUES ('" . $code . "','" . $issueDateToDate . "','" . $exBillNo[1] . "'," . $merId[0] . ",'" . $exShopName[1] . "'," . $amount . ",'" . $dueDateToDate . "','" . $dataJsonPeak['PeakReceipts']['resCode'] . "','" . $dataJsonPeak['PeakReceipts']['resDesc'] . "',CURRENT_TIMESTAMP()";

                    } else {
                        foreach ($dataJsonPeak['PeakReceipts']['receipts'] as $itemReponse) {
                            if ($itemReponse['resCode'] != 200) {
                                $insertQuery = "INSERT INTO ecom_receipts(code, issued_date, bill_no,mer_id, shop_name, amount_send,peak_due_date,peak_res_code,peak_res_desc,peak_code,peak_desc,record_date) " .
                                    "VALUES ('" . $code . "','" . $issueDateToDate . "','" . $exBillNo[1] . "'," . $merId[0] . ",'" . $exShopName[1] . "'," . $amount . ",'" . $dueDateToDate . "','" . $dataJsonPeak['PeakReceipts']['resCode'] . "','" . $dataJsonPeak['PeakReceipts']['resDesc'] . "','" . $itemReponse['resCode'] . "','" . $itemReponse['resDesc'] . "',CURRENT_TIMESTAMP())";
...
//                                $paymentAmount = $itemReponse['paymentAmount'];
                                ...
                                foreach ($itemReponse['products'] as $productPeak) {
                                    $insertProductPeakItem = "INSERT INTO ecom_receipts_peak_item(bill_no,item_date,peak_id, product_id, product_code, quantity, price, discount) VALUES " .
                                        "('" . $exBillNo[1] . "'," . $issuedDate . ",'" . $productPeak['id'] . "','" . $productPeak['productId'] . "','" . $productPeak['productCode'] . "'," . $productPeak['quantity'] . "," . $productPeak['price'] . "," . $productPeak['discount'] . ")";

                                    $customEntityManager->getConnection()->query($insertProductPeakItem);
                                }
                                $insertQuery = "INSERT INTO ecom_receipts(code, issued_date, bill_no, mer_id, shop_name, amount_send, pre_tax_amount, vat_amount, amount_peak, peak_due_date, peak_res_code, peak_res_desc, peak_code, peak_desc, peak_status, peak_online_link, record_date) " .
                                    "VALUES ('" . $code . "','" . $issueDateToDate . "','" . $exBillNo[1] . "'," . $merId[0] . ",'" . $exShopName[1] . "'," . $amount . ",".$preTaxAmount.",".$vatAmount."," . $paymentAmount . ",'" . $dueDateToDate . "','" . $dataJsonPeak['PeakReceipts']['resCode'] . "','" . $dataJsonPeak['PeakReceipts']['resDesc'] . "','" . $itemReponse['resCode'] . "','" . $itemReponse['resDesc'] . "','" . $status . "','" . $onlineViewLink . "',CURRENT_TIMESTAMP())";
    ...
    /**
     * @Route("/check/ecom/receipts/info",name="check_ecom_receipts")
     */
    public function checkEcomReceiptsInfo(Request $request)
    {
//        $entityManager = $this->getDoctrine()->getManager('default');
        $customEntityManager = $this->getDoctrine()->getManager('custom');
        $sqlPeakRequest = "SELECT bill_no,amount_send,amount_peak FROM ecom_receipts where result_amount is null AND result_quantity is null";
        $billNotTest = $customEntityManager->getConnection()->query($sqlPeakRequest);
        $dataExisted = json_decode($this->json($billNotTest)->getContent(), true);
        ...
        if ($dataExisted == []) {
            ...
            foreach ($dataExisted as $billItem) {
                ...
                $countSendProductQty = 0;
                $countPeakProductQty = 0;
                $sqlSendItem = "SELECT product_id,quantity FROM ecom_receipts_send_item " .
                    "WHERE bill_no='" . $billItem['bill_no'] . "'";
                $sendProductItem = $customEntityManager->getConnection()->query($sqlSendItem);
                $products = json_decode($this->json($sendProductItem)->getContent(), true);
                ...
                $sqlPeakItem = "SELECT product_code,quantity FROM ecom_receipts_peak_item " .
                    "WHERE bill_no='" . $billItem['bill_no'] . "'";
                $peakProductItem = $customEntityManager->getConnection()->query($sqlPeakItem);
                $peakProducts = json_decode($this->json($peakProductItem)->getContent(), true);
                ...
                ////////////////////////////////////////////////////////////////////////////////////////////////////////
                $updateResult = "UPDATE ecom_receipts SET product_quantity=" . $countSendProductQty . ",peak_product_quantity=" . $countPeakProductQty . ",result_quantity='" . $resultQuantity . "',result_amount='" . $resultAmount . "' " .
                    "WHERE bill_no='" . $billItem['bill_no'] . "'";
